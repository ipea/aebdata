...
list_themes()
```
```{r list-themes-knit, echo = FALSE, eval = aebdata:::test_connection_aeb("temas")}
list_themes() |>
...
```
```{r list-series-knit, echo = FALSE, eval = aebdata:::test_connection_aeb("temas") && aebdata:::test_connection_aeb()}
list_series(theme_id = c(41,49)) |>
  knitr::kable(format = "html")
```
...
```{r get-series, eval = aebdata:::test_connection_aeb()}
...
```
```{r download-141-knit, echo = FALSE, eval = exists("downloaded_series")}
head(downloaded_series$`240`) |>
  knitr::kable(format = "html")
```
