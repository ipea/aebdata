test_that("search works", {
  ...
  expect_equal(try_or_skip(search_series("militares")), rds$simple)
})
...
test_that("require_all works", {
  ...
  skip_if_not_installed("stringi")
  rds <- readRDS(test_path("_data", "search.rds"))
  expect_equal(try_or_skip(search_series(c("militares", "cargos"), require_all = TRUE)),
               rds$requireall_true)
})
...
test_that("case_insensitive works", {
  ...
  skip_if_not_installed("stringi")
  rds <- readRDS(test_path("_data", "search.rds"))
  expect_equal(try_or_skip(search_series("CAP ", case_insensitive = FALSE)),
...
test_that("return empty", {
  skip_if_api_offline()
  skip_if_not_installed("stringi")
  expect_equal(try_or_skip(search_series("miiltares")),
